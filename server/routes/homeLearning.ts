...
const buildAssistantFallback = (
  ...
  return {
    ...
    reply:
      "ابدأ بجلسة قصيرة لا تتجاوز ثلاث دقائق تركز على الصوت الصعب. قف أنت وطفلك أمام مرآة، ووضّح له حركة اللسان والشفاه ببطء، ثم اطلب منه تقليدك بثقة. بعد كل محاولة ناجحة، عزّزها بعبارة تشجيع وسجّل التقدّم في دفتر صغير.",
    simplifiedReply: "جلسة مرآة لمدة 3 دقائق + تقليد بطيء وتشجيع بعد كل محاولة.",
    voiceEnabled: body.modality !== "text",
    storedAt,
    cues: [
      "أظهر لطفلك موضع اللسان أمام المرآة",
      "قلّل السرعة إلى نصف الوتيرة المعتادة",
      "شجع على أخذ نفس عميق قبل النطق",
    ],
    nextActions: [
...
export const handlePostHomeLearningAssistantMessage: RequestHandler = async (req, res) => {
  ...
  if (!sanitizedMessage) {
    const fallback = buildAssistantFallback(body, {
      reply:
        "للبدء ساعدني بذكر الصوت أو الكلمة التي يواجه طفلك صعوبة في نطقها مع توضيح الموقف الذي تظهر فيه المشكلة، كي أوجّهك بخطوات دقيقة.",
