...
### تشغيل مساعد Gemini الذكي

لتفعيل المساعد الذكي المبني على Gemini، اتبع الخطوات التالية:

1. أنشئ ملفًا باسم `.env` في جذر المشروع (نفس مجلد `package.json`).
2. أضف السطر التالي مع مفتاح Google Generative AI الخاص بك:

   ```bash
   GEMINI_API_KEY=ضع_المفتاح_هنا
   ```

3. احتفظ بالملف محليًا ولا تضفه إلى نظام التحكم في الإصدارات.
4. أعد تشغيل الخادم (`npm run dev` أو `npm run start`) حتى يقرأ الإعداد الجديد.

تم تحديث الخادم لتحميل القيم من `.env` تلقائيًا عبر `dotenv/config`، لذا يكفي وضع المفتاح في الملف السابق لتفعيل التكامل.
