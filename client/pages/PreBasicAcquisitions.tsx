...
export default function PreBasicAcquisitions() {
  ...
  const ColorGame = ({
    ...
    const handleAnswer = (selectedIndex: number) => {
      const isCorrect =
        selectedIndex === questions[currentQuestion].correct % 4;
    ...
    if (showResult) {
      return (
        <div className="text-center space-y-6">
          ...
          <p className="text-lg">
            النتيجة: {score} من {instructions.length}
          </p>
          <div className="flex gap-4 justify-center">
            <Button
              onClick={() => {
      ...
      <div className="space-y-6">
        <div className="text-center">
          ...
          <div className="flex gap-2 justify-center">
            <Button
              onClick={() =>
                speakArabic(instructions[currentInstruction]?.text)
              }
            >
              <Volume2 className="w-4 h-4 ml-2" />
              استمع للتعليمة
            </Button>
            <Button variant="outline" onClick={checkRelation}>
              تحقق
            </Button>
          </div>
        </div>
          ...
          {categories.map((category) => (
            ...
            >
              <CardHeader className="text-center">
                <div
                  className={`${category.color} text-white p-6 rounded-xl w-20 h-20 mx-auto mb-4 flex items-center justify-center group-hover:scale-110 transition-transform`}
                >
                  {category.icon}
                </div>
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  <Progress
                    value={gameProgress[category.id] || 0}
        ...
        <Card className="mt-8 border-2 border-blue-200 bg-blue-50">
          ...
          <CardContent>
            <div className="grid md:grid-cols-2 gap-6 text-blue-700">
              <div>
                <h4 className="font-semibold mb-2">مر��لة التعلم:</h4>
