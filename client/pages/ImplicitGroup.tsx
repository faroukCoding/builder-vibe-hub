...
export default function ImplicitGroup() {
  ...
  const CategoryGame = ({
    ...
    const handleAnswer = (selectedItem: any) => {
      const isCorrect =
        selectedItem.name === questions[currentQuestion].correct.name;
    ...
    const renderShape = (shapeType: string) => {
      ...
      switch (shapeType) {
        case "circle":
          return (
            <div className={`${commonClasses} bg-blue-500 rounded-full`} />
          );
        case "square":
        ...
        case "triangle":
          return (
            <div
              className={`${commonClasses} bg-green-500`}
          ...
          {categories.map((category) => (
            ...
            >
              <CardHeader className="text-center">
                <div
                  className={`${category.color} text-white p-4 rounded-xl w-16 h-16 mx-auto mb-4 flex items-center justify-center text-2xl group-hover:scale-110 transition-transform`}
                >
                  {category.icon}
                </div>
